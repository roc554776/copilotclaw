...
- 中央集権的なプロセス（gateway）として、ポートを固定して、そこにサーバーを起動する
  - これで多重起動が防がれる
- このサーバーは、以下を持つ
  - dashboard（ページ。user input と reply をペアでリストする）
  - user input を post するエンドポイント（user input はキューイングされる）
  - user input を find one するエンドポイント（FIFO, なければ即時ないことが返る）
  - reply を post するエンドポイント（user input の id と reply メッセージをポスト）
  - healthz
