...
<!-- 2026-03-31 -->
## premium request クォータの正確な取得

- premium request のクォータ残量が正確に取得できるようにしてください。
  - GitHub の認証情報が直接使えるケース（gh 等を使っているケース）では、 GitHub API を直接叩いて、クォータ残量を取得してみる感じにしてください。
    - `/users/{{USERNAME}}/settings/billing/premium_request/usage` 的な感じ？？
  - エラーがでるなどして、取得できないケースでは、従来のやり方に fallback しましょう。
  - 理由: 従来のやり方だと恐らく正確な残量が取得できていないと思われるため。
  - なお、モデル一覧とプレミアムリクエスト乗数も同様です。
    - ただし、こちらの場合は、 api から取得したものと、従来のやり方で取得したものを区別して両方表示するようにしてください。
