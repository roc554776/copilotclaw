...
関連ファイル:
- `docs/requirements/channel-status-and-events.md` — チャンネルステータス・イベント抽象化・エージェント識別に関する要求（2026-04-14 追加）
- `docs/requirements/custom-agents.md` — Custom Agents 要求（ツール割り当て整理を含む）
...
- 全ての正規の呼び出し元は `sender` を明示しており、省略時のフォールバック動作に依存しているコードは存在しない
...
- cron は定期的にチャンネルにメッセージを送る仕組みとして実現する（`session.send()` を直接呼ばない — プレミアムリクエスト消費を最小化するため）
...
- モデル切り替えはあくまで設定値を変えるだけ。turn run を強制停止しない。次回 turn run 開始時に設定値のモデルを適用する（未実現 — 現状はモデル変更時にセッションが捨てられている）
...
### Req: メッセージ消費とセッションステータス管理のバグ修正（v0.64.0 で大部分実現済み。startPhysicalSession ack と IPC reconnect flush 順序は未実現）
