...
- 各 cron ジョブはジョブごとに disabled フラグで無効化できる（v0.42.0 で実現済み。フラグ名の `enabled` → `disabled` 変更は未実現）
