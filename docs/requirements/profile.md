...
### Req: Profile ごとの認証情報設定

profile ごとに GitHub Copilot の認証情報を変更できるようにする。

- 認証設定を config ファイルに持つ
  - シークレット（トークン本体）は config ファイルに直接書かない（環境変数名やファイルパスで間接参照する）
- 認証タイプ:
  - gh auth — `gh auth login` で取得済みの認証情報を profile 別に指定する。user を設定した場合は `gh auth token --user {{user}}` で特定アカウントのトークンを取得する
  - PAT — Fine-grained Personal Access Token (`github_pat_` プレフィクス) を使用する
  - OAuth — 将来対応。ユーザーが自分で OAuth App を登録し client_id を config に設定する方式
- 設定はオプショナル。デフォルトは GitHub Copilot CLI が使用する既存の認証情報を使う（`useLoggedInUser: true`）
- 認証設定は `auth.github.*` の名前空間に配置し、GitHub の認証情報であることを明確にする
