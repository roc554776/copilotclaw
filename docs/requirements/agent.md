...
### Req: Gateway-Agent 責務の再配置（部分実現 — プロンプト・agent 構成は v0.40.0 で移行済み、抽象セッション管理は未実現）
...
- 各種設定値（custom agent 構成、SYSTEM_REMINDER）も同様に gateway → agent の IPC 経由に移行する — **v0.40.0 で実現済み**
