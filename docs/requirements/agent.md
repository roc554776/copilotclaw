...
- gateway が停止している間に発生した情報（セッションイベント、agent 応答メッセージ、システムプロンプト等）は失われてはならない。gateway に再接続された時に全ての情報が復元される必要がある
- gateway に再接続されないまま agent process が停止した場合でも、再起動後に情報が失われずに再開できなければならない
...
- ゾンビプロセスを残さないこと — v0.66.0 で client.stop()/forceStop() を全 stop パスに追加し対応
...
### Req: Gateway-Agent 責務の再配置（v0.50.0 で実現 — 設定値は v0.40.0、抽象セッション管理は v0.49.0+v0.50.0）
...
- 抽象セッション管理（channel binding、suspended 状態管理、revive 判定）を gateway process 側に移動する — **v0.49.0+v0.50.0 で実現済み**
- agent process は物理セッション（Copilot SDK session）の作成・実行・停止のみを担当する — **v0.50.0 で実現済み（物理セッション状態追跡、workspace bootstrap を gateway に移行。モデル選択は gateway が主導し agent は fallback のみ）**
- `assistant.message` のチャンネル反映を gateway 側に移動する — gateway は既に `session_event` として `assistant.message` を受信しているので、gateway の `onSessionEvent` で処理すべき。agent 側の直接 `channel_message` 送信は wrong-side ロジック — **v0.62.0 で実現済み**
...
### Req: session.idle での subagent 停止と親 agent idle の区別（v0.65.0 で実現済み — agent 側セッションループで backgroundTasks 付き session.idle をスキップ、30分タイムアウト付き安全弁）

session.idle イベントが subagent の停止に起因するものか、親 agent（抽象セッションに直接紐づく channel-operator）の idle なのかを区別し、処理が壊れないようにする。
...
### Req: physical session の常時保持（v0.58.0 で実現済み）
...
### Req: 連続した turn 列の概念の導入（v0.58.0 で実現済み）
...
- end turn run は物理セッションを即時 disconnect し、次のメッセージではそれを resume して使う。disconnect したセッションを捨てると会話がレジュームできない。Archive session は disconnect してセッションを本当に捨てる機能（未実現 — 現状は stopPhysicalSession を呼んでおり disconnect ではない）

### Req: セッション status の細分化（v0.58.0 で実現済み）
