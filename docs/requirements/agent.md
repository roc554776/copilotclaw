...
### Req: Gateway-Agent 責務の再配置（未実現）

gateway process だけ最新版を起動しても最新機能が使えるように、設定値と抽象セッション管理を gateway 側に移動する。

- システムプロンプト（CHANNEL_OPERATOR_PROMPT 等）を gateway 側の agent 用モジュールで管理し、IPC 経由で agent に送る
- 各種設定値（custom agent 構成、ツール定義、hook 設定等）も同様に gateway → agent の IPC 経由に移行する
- 抽象セッション管理（channel binding、suspended 状態管理、revive 判定）を gateway process 側に移動する
- agent process は物理セッション（Copilot SDK session）の作成・実行・停止のみを担当する
