...
### Req: Gateway-Agent 責務の再配置（部分実現 — 設定値は v0.40.0、抽象セッション管理は v0.49.0 で部分移行）
...
- 抽象セッション管理（channel binding、suspended 状態管理、revive 判定）を gateway process 側に移動する — **部分実現（v0.49.0 で SessionOrchestrator を gateway に新設したが、agent 側にも抽象セッション状態の二重管理が残存）**
- agent process は物理セッション（Copilot SDK session）の作成・実行・停止のみを担当する — **未実現（モデル選択ポリシー、物理セッション状態追跡、リマインダーポリシー、workspace bootstrap 等が agent に残存）**
