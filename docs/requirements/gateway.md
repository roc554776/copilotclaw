...
### Req: セッション一覧ページの構造（v0.36.0 で再調査・確認済み）
...
### Req: sessions リンク表記（v0.36.0 で修正済み）

`/sessions` へのリンクの表記を全ページで `All sessions →` にする。

- `/sessions` ページが抽象セッション主体の階層表示であるため、「physical sessions」という表現は不適切
- 対象: `/status` ページ、dashboard モーダル、フォールバック HTML

### Req: Effective System Prompt の表示

物理セッションで実際に適用される effective system prompt（将来的には original system prompt から改変される可能性あり）を API と dashboard で参照できるようにする。
