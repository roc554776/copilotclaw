...
- `disabled`: ジョブの無効化フラグ（デフォルト: `false`）。`true` に設定するとスケジューラに登録されない。旧 `enabled` フィールド（デフォルト `true`）からの変更（未実現）。変更理由: デフォルトが `true` のフラグはバグの温床になるため、否定形でデフォルト `false` にする
- 動的な config 再読み込みを `cron reload` コマンドで提供する（未実現）
