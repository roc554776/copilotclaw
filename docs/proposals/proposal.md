...
<!-- TODO: 未実装 — 現在は内蔵 chat が gateway に密結合 -->
...
Gateway の責務は user message の管理、agent プロセスの ensure、チャットシステムの提供。Agent 内部のチャンネル管理には関与しない。

```
Gateway: user message 受信時
...
**今後の課題:**
- Channel アーキテクチャ再設計（内蔵 chat を channel プラグインとして分離）
