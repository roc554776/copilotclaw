...
| `mockTools` | `boolean?` | `COPILOTCLAW_MOCK_TOOLS` | 開発用モックツールモード（デフォルト: false） |
...
- デフォルトモデル選択設定（`model` / `zeroPremium` / `mockTools` config + doctor チェック）
