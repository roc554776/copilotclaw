...
- gateway start 時に agent process を ensure する（プロセスの生存確認 + バージョンチェック、なければ spawn）- gateway stop 時に agent process は停止しない（agent session は高コストなので gateway restart 後もそのまま再利用するため）- dashboard ページによるチャット UI（メッセージ入力 + user message / reply の時系列表示）
...
Gateway の責務は user message の管理、agent process の ensure（start 時のみ）、チャットシステムの提供。Agent 内部の session 管理には関与しない。
...
- SSE の `status_update` イベントまたは `/api/status` ポーリングで session 状態を監視する
- processing 開始時にインジケータを表示、waiting に戻った時（または新しいメッセージ到着時）に非表示にする
- 表示例: チャットバブルのスタイルでドットアニメーション（`typing...` のような一般的なパターン）
...
- gateway と agent は独立してバージョニングされる（monorepo 内の個別パッケージ）
...
同一マシン上で複数の独立した copilotclaw インスタンスを実行するための仕組み。

- 環境変数 `COPILOTCLAW_PROFILE` で workspace を分離
- workspace ディレクトリ名に反映: `workspace-{{profile}}`
- 優先度: 低（当面は単一インスタンスで運用）
