...
`onPostToolUse` の `input.toolName` を確認し、subagent 内部の tool かどうかを判別する必要がある。SDK の `PostToolUseHookInput` には subagent コンテキストの明示的なフィールドがないため、実装時に以下の方法で判別する:
- `toolName` が `copilotclaw_receive_input` / `copilotclaw_send_message` / `copilotclaw_list_messages` のいずれかなら、channel-operator（親 agent）のコンテキストであると判断する
- ビルトインツール名のみの場合は、subagent の可能性がある — この場合はリマインドを挿入しない（安全側に倒す）
- ただし、SDK の hooks がそもそも subagent の tool 実行で発火するかどうかは要検証。hooks はセッションレベルで登録されるため、同一セッション内の subagent でも発火する可能性がある
...
**今後の課題:**
- Custom Agent 構成の導入（channel-operator + worker の 2 agent 体制、システムプロンプト安定化）
