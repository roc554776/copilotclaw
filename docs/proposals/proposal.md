...
- agent process 再起動後も channel → agent session の紐づけを維持するため、binding 情報（channelId ↔ sessionId + copilotSessionId）を永続化する
- 永続化先は gateway の store（既存の永続化インフラ）または agent 独自の永続化ファイル
...
- 抽象 Agent Session と物理 Copilot Session の分離（suspended 状態、channel binding 維持、reviveSession による自動復帰）
...
**今後の課題:**
- Channel binding の永続化（agent restart 後の channel → agent session 紐づけ維持）
