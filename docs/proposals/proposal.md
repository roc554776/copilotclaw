...
- dashboard ページによる user input / reply ペアの一覧表示
...
- テスト対象の後続機能が未実装の場合は `skip` 状態でテストを記述する
...
- Copilot SDK を用いた Agent の hello world（session idle loop による停止制御を含む）
- pnpm monorepo 構造（tsconfig strictest 相当の設定）

**今後の課題:**
- CLI インターフェースの実装
- ツール群（ファイル操作・シェル実行・検索・Git）の実装
