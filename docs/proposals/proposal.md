...
**今後の課題:**
- Profile 機能（同一マシン上の複数インスタンス分離、profile ごとの workspace・設定ファイル・gateway・agent 分離）
- 設定ファイル機能（`config.json` による動作設定、環境変数との優先順位ルール、ポート設定）
- Setup 時のポート自動選択（デフォルトポート使用中の場合に候補リストから空きポートを探索）
