...
### NFR: premium request クォータの正確な取得（v0.61.0 で実現済み）

GitHub API を直接叩いて premium request のクォータ残量とモデル情報を正確に取得する。

**クォータ取得:**
- GitHub の認証情報が直接使えるケース（gh 等を使っているケース）で、GitHub API を呼び出す
- エンドポイント例: `/users/{{USERNAME}}/settings/billing/premium_request/usage`（要調査）
- 取得に失敗した場合は従来のやり方（Copilot SDK 経由の IPC）に fallback する

**モデル一覧・プレミアムリクエスト乗数:**
- GitHub API からもモデル一覧と乗数を取得する
- API 由来のデータと従来（SDK 由来）のデータを区別して両方表示する

**実装配置（Mechanism）:**
- GitHub API 呼び出しは gateway process 側に置く（agent process は turn run 延命のための別プロセスであり、この責務は gateway に属する）
- gateway の `/api/quota` と `/api/models` エンドポイントで、GitHub API → SDK IPC の fallback チェーンを実装する
