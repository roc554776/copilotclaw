...
SDK の builtin tool list API を使って、custom agent 定義の `tools` フィールドに明示的な tool list を設定する。

```typescript
// 疑似コード（SDK の実際の API に合わせて実装する）
const builtinTools = await client.listBuiltinTools()

const channelOperatorDef = {
  name: "channel-operator",
  // ...
  tools: [
    ...builtinTools,
    copilotclawWaitToolDef,
    copilotclawListMessagesDef,
    copilotclawSendMessageDef,
    copilotclawIntentDef,  // 新規追加
  ],
}
...
`tools: null` を廃止し、`tools: [...]` の明示指定に変更する。SDK の builtin tool list API の呼び出しタイミングは `PhysicalSessionManager` の初期化時（client start 後）とする。
