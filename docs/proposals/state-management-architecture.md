...
| send queue | v0.79.0 で解消済み: ACK プロトコル実装。`pendingAckIds: Set<string>` + `_queueId` 自動付与 + flush 後 ACK 待ち + gateway からの `message_acknowledged` IPC 受信後にディスク削除。MIN_AGENT_VERSION を 0.79.0 に引き上げ |
