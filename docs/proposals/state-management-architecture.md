...
| config push | v0.83.0 で gateway 側 reducer 化済み（`config-push-events.ts` / `config-push-reducer.ts`）。`ConfigSet` / `StreamConnected` / `StreamDisconnected` event。`SendConfig` command。`AgentManager.dispatchConfigPushEvent()` が reducer を呼び出し、stream が接続済みの場合に `SendConfig` command を実行する |
...
**現状（v0.83.0 実装済み: gateway 側 reducer 化）**

`packages/gateway/src/config-push-events.ts` / `config-push-reducer.ts` に ConfigPush reducer を実装。`AgentManager` が `dispatchConfigPushEvent()` 経由で reducer を呼び出す。

**world state**

```typescript
interface ConfigPushWorldState {
...
- `ConfigSet { config }` — gateway が `setConfigToSend()` を呼んだとき
- `StreamConnected` — IPC stream が接続されたとき
- `StreamDisconnected` — IPC stream が切断されたとき
- `SendConfig { config }` — agent に config を送信する command

**2 つの push パス**

- `ConfigSet` 受信時に `connected=true` → `SendConfig` を即時発行
- `StreamConnected` 受信時に `config !== null` → `SendConfig` を発行（接続時の pending config 送信）

**v0.82.0 での経緯と解決**

v0.82.0 では agent 側 `ipc-reducers.ts` に型定義・reducer を追加したが、gateway 側 `AgentManager` が agent package を import する逆依存が生じるため dead code として削除した。v0.83.0 で reducer を **gateway 側**（`config-push-events.ts` / `config-push-reducer.ts`）に配置することで package 依存制約を解消し、10/10 subsystem の reducer 化を達成した。
