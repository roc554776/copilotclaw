...
`packages/gateway/src/agent-config.ts` の `customAgents` 定義で `tools: null` が指定されており、SDK が暗黙的にツールを割り当てる。この暗黙割り当てでは `copilotclaw_*` ツールが割り当てられないケースが発生している。
...
`tools: null` を廃止し、`tools: [...]` の明示指定に変更する。`client.rpc.tools.list({ model })` は `ToolsListParams.model` の有無で結果が変わる（SDK 定義: "Optional model ID — when provided, the returned tool list reflects model-specific overrides"）ため、**結果を cache してはいけない**。モデルごとに異なる可能性があるため、session 作成直前（`createSession` を呼ぶ直前、`resolvedModel` が確定した時点）で、その model を引数に渡して都度呼び出す。reinject による再作成、end-turn-run 後の再 `createSession`、dynamic model switch 後の再作成など、session を新しく作る経路すべてで都度取得する。
