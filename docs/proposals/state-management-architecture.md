...
**session-scoped SSE の Last-Event-ID reconnect replay 設計（未実現）**

channel / global の replay は SSE broadcaster の in-memory replay buffer 設計（`ChannelScopedSseStatePerChannel.recentEvents` / `GlobalSseState.recentEvents`）を使うが、session-scoped SSE は SSE broadcaster の actor model とは独立した簡易設計として実装する。理由: session event は既に `SessionEventStore` の DB に永続保存されており、追加の in-memory buffer を持たなくてよいため。

設計の要素:

- **`SessionEventStore.listEventsAfterId(sessionId, afterId, limit?)`**: 指定した物理セッション ID に属する event のうち、`id > afterId` となるものを昇順に `limit` 件以内で返す。目的は reconnect 時の catch-up 配信用 query 窓口を提供すること。signature:

  ```typescript
  listEventsAfterId(
    sessionId: string,
    afterId: number,
    limit?: number  // デフォルト: 500。省略時は実装定数 SESSION_REPLAY_LIMIT を使用
  ): Promise<SessionEvent[]>
  ```

- **SSE frame の `id:` line**: `session_event_appended` event の SSE frame に `id: <sessionEventId>` line を付与する。`sessionEventId` は `SessionEventStore` の INTEGER PRIMARY KEY AUTOINCREMENT 値。これにより EventSource がその値を記憶し、reconnect 時に `Last-Event-ID` header として自動送信する。sse-broadcaster の `broadcastToSession` または endpoint handler で各 frame に `id:` を追加する

- **catch-up 配信の流れ**: `/api/sessions/:id/events/stream` endpoint で SSE 接続を受け付ける際、`Last-Event-ID` request header を parse する。値が存在し数値として有効な場合、`SessionEventStore.listEventsAfterId(sessionId, lastEventId)` で DB から missed event を取得し、接続確立直後に catch-up として順次送信する。その後は通常の `broadcastToSession` 経由の push に切り替える
...
- **replay 上限**: catch-up クエリの上限は `SESSION_REPLAY_LIMIT`（実装既定: 500）とする。取得件数が上限に達した場合は打ち切り、その旨をサーバーログに記録する。`limit?` パラメータを明示する理由はテスト容易性のためであり、production では定数を使用する

- **SSE broadcaster との関係**: `SseBroadcasterEvent.ClientConnected` の `scope` は現在 `"channel" | "global"` であり、`"session"` は含まない。session-scoped SSE の catch-up は SSE broadcaster の reducer を経由せず、endpoint handler が直接 `SessionEventStore` に query して送信する独立した実装とする。これにより channel / global の replay buffer 設計への影響を与えない
