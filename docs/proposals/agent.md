...
そのため、`toolName` で判別する。`copilotclaw_wait` は parent agent (channel-operator) のみに与えられる唯一の専用ツールであり、subagent (worker) には提供されない。`copilotclaw_send_message` と `copilotclaw_list_messages` は worker も使用するため、これらでのゲートは不可。
