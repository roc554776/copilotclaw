...
**現状の懸念:**
- gateway 停止時に IPC stream が切断される
- `copilotclaw_wait` は IPC 経由で gateway に `drain_pending` を送信してブロックするが、IPC 切断時にエラーが発生する可能性がある
- `copilotclaw_wait` のエラー不可侵性（エラーを返さない設計）により、IPC エラーは keepalive レスポンスとして処理されるはずだが、全経路で物理セッションが安全に維持されるか検証が必要

**対応方針:**
- `copilotclaw_wait` が IPC 切断中でもエラーを返さず、keepalive cycle を継続する設計を維持する
- gateway 再接続時に自動的に通常動作に復帰する
- IPC 切断が物理セッション内部のツール呼び出しループを終了させないことを保証する
...
### Gateway-Agent 責務の再配置（部分実現 — 設定値は v0.40.0 で移行済み、抽象セッション管理は未実現）
