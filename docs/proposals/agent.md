...
- end turn run は物理セッションを即時 disconnect する（停止ではなく disconnect）。copilotSessionId を保持し、次回メッセージで resumeSession により再開する。disconnect したセッションを捨ててはならない（コンテキストが消える）。Archive session は disconnect してセッションを本当に捨てる機能。（未実現 — 現状は stopPhysicalSession を呼んでおり disconnect ではない）
