...
SDK の `onPostToolUse` hook は subagent のツール実行では発火しない（v0.39.0 の debug ログによる実測で確認済み）。hook が発火するのはメインエージェント（channel-operator）のツール実行のみ。したがって、subagent 排除のための `toolName` ゲートは不要。

ただし、全ツールでリマインドを送るとコンテキストを浪費するため、`toolName === "copilotclaw_wait"` でゲートして発火頻度を制限している。この制限は subagent 排除ではなくコンテキスト節約が目的。
