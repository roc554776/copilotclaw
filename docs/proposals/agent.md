...
**gateway 再起動時の二重セッション問題（未実現）:**

物理セッション延命自体は機能するが、gateway 再起動時に延命した物理セッションが正しく再利用されない。
...
この問題は「gateway の停止が物理セッションの破壊につながる設計は常に許されない」の間接的な違反。物理セッションは壊れないが、gateway 再起動により延命した物理セッションが無意味になり、リソースが浪費される。
...
**agent process 監査結果 — gateway に移行すべきロジック（未実現）:**
...
**v0.49.0 移行後のコードレビューで発見された残存問題:**

v0.49.0 で CRITICAL（チャンネルバインディング、suspended 永続化、復活、状態管理）と HIGH（バックオフ、stale 検出、max age、pending ポーリング）の項目は gateway 側 SessionOrchestrator に移行された。しかし、agent process にまだ以下のロジックが残存しており、「agent process は物理セッションのみ担当」は未実現である。

CRITICAL — 抽象セッション状態の二重管理（未実現）:
- `AgentSessionInfo`（status, `cumulativeInputTokens`, `cumulativeOutputTokens`, `physicalSessionHistory`）が agent に残存
- `suspendSessionState()` が agent 側でトークン累積と履歴管理を実行
- gateway の `SessionOrchestrator` がまったく同じことをしている — 完全な二重管理
- agent 側の `getSessionStatuses()` が IPC `status` RPC に使われ、dashboard 表示に影響するが、同じ情報は orchestrator にもある

CRITICAL — gateway 再起動時の二重セッション（未実現）:
- gateway 停止 → `orchestrator.suspendAllActive()` で全セッション suspended
- gateway 再起動 → 新 stream 接続 → `checkAllChannelsPending()` → pending 検出 → `startSessionForChannel()` → `orchestrator.hasActiveSessionForChannel()` は false（全部 suspend 済み）→ 新物理セッション開始
- しかし agent の旧物理セッションはまだ動いている → 同一チャンネルに 2 つの物理セッションが並走
- 両方の物理セッションが `drain_pending` を呼び、メッセージの競合が発生する
- v0.49.0 以前は agent が `channelBindings` を持っていたため `hasSessionForChannel` が agent 側で判定できたが、移行後はこのチェックが消えた
- raw requirement「gateway の停止が物理セッションの破壊につながる設計は常に許されない」に対する間接的な違反。gateway 再起動により、延命した物理セッションが無意味になる

HIGH — モデル選択ポリシーが agent にある（未実現）:
- `resolveModel()` が zeroPremium 判定、billing multiplier ソート、フォールバック（gpt-4.1）を実行
- gateway が `model` と `zeroPremium` を config で送っているが、実際の選択アルゴリズムは agent 内にある
- gateway を更新してモデル選択ポリシーを変えても、agent 再起動なしには反映されない

HIGH — keepalive タイムアウトが agent にハードコード（未実現）:
- `DEFAULT_KEEPALIVE_TIMEOUT_MS = 25 * 60 * 1000` が `tools/channel.ts` にハードコードされている
- gateway config（`prompts` オブジェクト）に含まれておらず、gateway から制御できない

MEDIUM — 物理セッション状態の詳細追跡が agent にある（未実現）:
- `PhysicalSessionSummary` の更新（currentState, currentTokens, tokenLimit, totalInputTokens/OutputTokens, latestQuotaSnapshots）
- `SubagentInfo[]` の追跡（subagent.started/completed/failed イベントによる状態更新）
- これらのイベントは既に `session_event` として gateway に転送されているため、gateway 側でイベントから再構築可能

MEDIUM — リマインダーポリシーが agent にハードコード（未実現）:
- `reminderState` によるコンテキスト使用率 10% 刻みの閾値判定
- `session.compaction_complete` 後の即時リマインドフラグ
- ポリシー判定が agent にあり、gateway から制御できない

LOW — workspace bootstrap が二重実行される可能性（未実現）:
- agent: `ensureWorkspaceReady()` が物理セッション開始前に実行（git init、bootstrap files 作成、initial commit）
- gateway: `ensureWorkspace()` が daemon.ts で起動時に実行
- 同一の workspace に対して二重に初期化処理が走る可能性がある
...
- **gateway 再起動（agent 生存）時に旧物理セッションとの二重セッションが発生しないか**
