...
### SDK CLI 子プロセスのゾンビ化（v0.66.0 で部分対応、根本修正は未実現）

agent プロセス停止時に、SDK が spawn した CLI 子プロセス（`@github/copilot/index.js`）がゾンビとして残る。

**根本原因:**

CopilotClient を物理セッションごとに新規作成している（`startPhysicalSession` のたびに `createClient()`）。CopilotClient は agent process 全体で 1 つでよい。セッションごとにクライアントを作ると CLI プロセスが増殖し、`session.disconnect()` や `client.stop()` の呼び漏れでゾンビ化する。

**現状の対応（v0.66.0）:**

- 全 stop パスで `client.stop()` / `forceStop()` を呼ぶようにした
- pooled client も含めて停止

**根本修正（未実現）:**

- CopilotClient を agent process 全体で 1 つにする（シングルトン）
- physical session id を自分で生成し、SDK の `createSession` / `resumeSession` に渡す
- `copilotSessionId` という表現を廃止し、physical session id に統一する（同じものを指しているため）
- end-turn-run: `session.disconnect()` で物理セッションを切断（クライアントは止めない）。physical session id を保持し、次回 `resumeSession` で再開
- archive session: `session.disconnect()` + physical session id を破棄（コンテキストを捨てる）
- agent 停止時: 唯一のクライアントに `client.stop()` / `forceStop()` を呼ぶ
- `CopilotSession` の `await using` 構文を使い、スコープ離脱時の `disconnect()` 漏れを防ぐ
...
`session.send()` は session の開始時以外には使わない（コスト最小化の原則）。CLI の 30 分 idle timeout を回避するため、`copilotclaw_wait` tool の内部で input をポーリングしながら待機する。tool が実行中の間はセッションは active 扱いとなり timeout しない。
...
- end turn run は物理セッションを `session.disconnect()` で切断する（クライアントは止めない）。physical session id を保持し、次回メッセージで `resumeSession(physicalSessionId)` により再開する。disconnect したセッションの physical session id を捨ててはならない（コンテキストが消える）。Archive session は `session.disconnect()` + physical session id を破棄する機能。（未実現 — 現状は stopPhysicalSession を呼んでおり、クライアントがセッションごとに作られている）
