...
**未実現:**
- トークン消費データへの乗数保存 — `assistant.usage` イベント保存時にプレミアムリクエスト乗数も記録する。現状は乗数を保存しておらず、モデル廃止・乗数変更時に過去の指数が計算不可能
- トークン消費時系列 API — 期間とタイムインスタンス数を指定して時系列データを返す。モデル別消費量、指数、移動平均（時間幅指定可能）
- トークン消費グラフ UI — 時系列データをグラフで表示する新規ページ
