...
- 状態管理アーキテクチャ再設計 — 残り subsystem 全 reducer 導入・EventBus・backoff 永続化・dead code 削除（v0.82.0）:
  ...
  - agent `ipc-reducers.ts`: ConfigPush reducer を削除（v0.82.0 時点の設計判断: stateless で dead code、package 依存方向の制約により production wiring 不可。v0.83.0 で gateway 側に再実装し解決済み — 下記 Item G 参照）
...
- 状態管理アーキテクチャ再設計 — IPC 型付き union・ACK プロトコル・SSE 正規化・タイムライン・reconcile request-response・double drain 完全排除・ConfigPush reducer 化（v0.83.0）:
  ...
  - Item G: ConfigPush reducer を gateway 側に実装（10/10 subsystem 完了）— `config-push-events.ts`（`ConfigPushWorldState` / `ConfigPushEvent` / `ConfigPushCommand`）と `config-push-reducer.ts`（`reduceConfigPush`）を新設。`AgentManager` の `configToSend` フィールドを `configPushState: ConfigPushWorldState` に置き換え、`setConfigToSend()` / `connectStream()` の "connected" / "disconnected" ハンドラを `dispatchConfigPushEvent()` 経由に変更。package 依存制約（`packages/gateway` → `packages/agent` 逆依存を回避）を解消。unit tests: `config-push-reducer.test.ts` に 12 tests 追加

**未実現:**
- 系全体の状態管理アーキテクチャ再設計（`docs/proposals/state-management-architecture.md`） — v0.83.0（Item G）で ConfigPush reducer 化が完了し、10/10 subsystem すべての reducer 化が達成された（詳細は「実装済み」節の v0.83.0 項を参照）
