<!-- Generated: 2026-03-27 | Updated: 2026-03-28 | Files scanned: 43 | Version: 0.40.0 | Token estimate: ~4000 -->
...
src/store.ts               — persistent store (Channel, Message, per-channel pending queue); SQLite via better-sqlite3 (WAL mode, foreign keys); tables: channels (id PK, createdAt, archivedAt nullable), messages (id PK, channelId FK, sender, message, createdAt; indexed by channelId+createdAt), pending_queue (autoincrement id, channelId FK, messageId FK; indexed by channelId); archiveChannel(id) sets archivedAt timestamp, unarchiveChannel(id) clears it; listChannels({ includeArchived }) filters archived channels by default; schema migration adds archivedAt column; StoreOptions accepts persistPath (SQLite file, default :memory:) and legacyJsonPath (one-time migration from store.json when DB is empty); close() method
