<!-- Generated: 2026-03-27 | Updated: 2026-03-31 | Files scanned: 45 | Version: 0.57.0 | Token estimate: ~5000 -->
...
src/pages/DashboardPage.tsx    — Chat dashboard (channel management, message list, input form, SSE events, status bar, logs panel); showArchived toggle for displaying archived channels; archive/unarchive buttons on channel tabs; mobile responsive (tab overflow-x: auto, flexShrink, modal min/max width); SSE auto-reconnect on error (3s delay); visibilitychange handler for message refresh; infinite scroll (loadOlderMessages on scroll near top, cursor-based pagination via `before` parameter); ChannelSettingsModal component: opened by clicking tab channel ID; displays/changes per-channel model setting (select from available models, null for global default), shows active physical session with stop button, shows cron jobs for the channel with reload button; channel model display/change via updateChannelModel, physical session stop via stopSession, cron job display via fetchCronJobs, cron reload via reloadCron; cron section has editable forms (add/edit/delete cron jobs with Save & Reload via saveCronJobs)
...
### Test Suites (484 total: 476 vitest + 8 Playwright)

```
Gateway vitest (360 tests)   — unit + E2E tests with mock agent (includes config, config-cli, config-migration, doctor, ipc-paths, setup, workspace, structured-logger, session-event-store, store, session-orchestrator, agent-manager, daemon-session-event-handler, daemon-lifecycle-handler tests); E2E tests use SessionEventStore for /api/token-usage endpoint coverage; session-orchestrator tests cover start/suspend/stop/revive, SQLite persistence, legacy migration, backoff, maxAge, suspendAllActive
