<!-- Generated: 2026-03-26 | Updated: 2026-03-26 | Files scanned: 31 | Version: 0.15.0 | Token estimate: ~1900 -->
...
- `onPostToolUse` — executes after each tool call; performs two checks:
  - Pending message detection: peeks channel for pending user messages, injects notification into additionalContext
  - System prompt reinforcement (v0.15.0+): fires periodic reminders via `<system>` tagged additionalContext when context usage crosses 10% increments or after compaction; only fires for `copilotclaw_*` tool names (built-in tools skipped to avoid subagent interference); maintains reminderState (needsReminder, lastReminderPercent, currentUsagePercent) to avoid firing on every tool call
