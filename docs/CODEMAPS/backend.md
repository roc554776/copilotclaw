<!-- Generated: 2026-03-27 | Updated: 2026-03-29 | Files scanned: 43 | Version: 0.48.0 | Token estimate: ~4400 -->
...
### Test Suites (442 total: 434 vitest + 8 Playwright)

```
Gateway vitest (297 tests)   — unit + E2E tests with mock agent (includes config, config-cli, config-migration, doctor, ipc-paths, setup, workspace, structured-logger, session-event-store, store tests); E2E tests use SessionEventStore for /api/token-usage endpoint coverage
Agent vitest (105 tests)     — unit tests with mock Copilot SDK session + IPC stream tests (includes structured-logger, token-resolver, ipc-stream tests; currentState tracking via onStatusChange, postToolUse log session ID tests); session-event-store tests include token usage aggregation (getTokenUsage)
