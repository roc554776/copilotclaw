<!-- Generated: 2026-03-27 | Updated: 2026-03-29 | Files scanned: 43 | Version: 0.49.0 | Token estimate: ~4400 -->
...
src/daemon.ts              — daemon entry point (ensureWorkspace + Store init + LogBuffer + OTel init + creates SessionEventStore + passes to startServer + periodic agent monitor every 30s); sets up IPC stream: agentManager.setStreamMessageHandler() routes agent messages (channel_message → store + SSE, session_event → SessionEventStore + subagent completion handling, system prompts → SessionEventStore, drain/peek/flush/list_messages → Store); onSessionEvent receives channelId from agent, detects subagent.completed/failed for direct calls (no parentToolCallId), inserts system message into channel + sends agent_notify (v0.43.0); agentManager.setConfigToSend() prepares config push (includes debug config and prompts from getAgentPromptConfig), agentManager.connectStream() after agent ensure; SessionOrchestrator with SQLite persistence (session-orchestrator.db) and legacy agent-bindings.json migration (v0.49.0); onStreamDisconnected suspends all active sessions via orchestrator.suspendAllActive(); cron scheduler loop: reads config.cron, skips jobs with enabled===false (v0.42.0), sets up setInterval per job, dedup check via store.hasPendingCronMessage(), sends cron message + agent_notify (v0.43.0); graceful shutdown calls orchestrator.close() + agentManager.closeStream()
...
src/session-orchestrator.ts — SessionOrchestrator: manages abstract session lifecycle (start, suspend, stop, revive); SQLite-backed persistence (abstract_sessions table, WAL mode); in-memory maps for sessions, channelBindings, channelBackoff (backoff is ephemeral, not persisted); persistSession() upserts on every mutation (startSession, suspendSession, stopSession, updateSessionStatus, updatePhysicalSession); suspendAllActive() suspends all non-suspended sessions (used on stream disconnect); loadFromDb() on construction; legacy migration from agent-bindings.json (one-time, renames to .migrated); close() method
...
src/agent-manager.ts       — IPC-based agent process ensure at gateway start (spawn, version check, force-restart); uses createRequire to resolve @copilotclaw/agent package path; ensureAgent returns old bootId on force-restart; waitForNewAgent polls until different bootId appears; checkCompatibility() and getMinAgentVersion() methods; getQuota(), getModels(), and getSessionMessages() proxy to agent IPC; MIN_AGENT_VERSION exported (0.49.0); semverSatisfies exported (used by doctor); agent stderr redirected to {{stateDir}}/data/agent.log on spawn (openSync append mode); COPILOTCLAW_GATEWAY_URL no longer set in spawn env (v0.35.0); IPC stream management: connectStream(), setStreamMessageHandler(), setConfigToSend(), notifyAgent(), closeStream(); notifyAgent(channelId) sends generic agent_notify (used for pending messages, subagent completion, etc.; v0.43.0, renamed from notifyPending); reconnectStream() private method closes existing stream and reconnects (called after spawning new agent or force-restart); onStreamConnected() and onStreamDisconnected() callback registration (v0.49.0); StreamMessageHandler interface: onSessionEvent receives channelId (v0.43.0) for routing agent messages (channel_message → store + SSE, session_event → SessionEventStore + subagent completion, system prompts → SessionEventStore, drain/peek/flush/list_messages → Store)
...
### Test Suites (437 total: 429 vitest + 8 Playwright)

```
Gateway vitest (345 tests)   — unit + E2E tests with mock agent (includes config, config-cli, config-migration, doctor, ipc-paths, setup, workspace, structured-logger, session-event-store, store, session-orchestrator tests); E2E tests use SessionEventStore for /api/token-usage endpoint coverage
Agent vitest (84 tests)      — unit tests with mock Copilot SDK session + IPC stream tests (includes structured-logger, token-resolver, ipc-stream tests; currentState tracking via onStatusChange, postToolUse log session ID tests); session-event-store tests include token usage aggregation (getTokenUsage)
