<!-- Generated: 2026-03-27 | Updated: 2026-03-27 | Files scanned: 41 | Version: 0.28.0 | Token estimate: ~3400 -->
...
src/daemon.ts              — daemon entry point (ensureWorkspace creates data/ + workspace/ dirs + Store init + LogBuffer creation + enableFileOutput to {{stateDir}}/data/gateway.log + console intercept + creates SessionEventStore(dataDir) + passes to startServer on resolvePort() + periodic agent monitor every 30s, max 3 retries)
...
src/session-event-store.ts — SessionEventStore: disk-based event storage (JSON Lines per session, storage cap enforcement), original/session system prompt storage; data stored in {{dataDir}}/events/ and {{dataDir}}/prompts/
src/observability-pages.ts — renderStatusPage() and renderEventsPage(): standalone HTML pages for system status and session events stream (flat/nested toggle, auto-scroll)
src/dashboard.ts           — HTML renderer (status bar with compatibility label, chat bubbles, channel tabs, input form, logs panel toggled via Logs button with stopPropagation to prevent status modal opening); status modal shows physical session details (with elapsed time, accumulated tokens in/out/total), cumulative tokens across physical sessions (v0.27.0), subagent sessions, premium requests, available models; quota display uses /api/quota with fallback to latestQuotaSnapshots from session data; showSessionDetail() fetches and displays copilot session context detail via /api/sessions/:sessionId/messages; modal includes "Open in new tab" link to /status page; physical sessions have "View events" link to /sessions/:id/events
...
~/.copilotclaw/                  (state dir, default profile)
  ...
  data/
    ...
    events/                  — session event files ({{sessionId}}.jsonl, JSON Lines format; storage cap enforced by SessionEventStore)
