...
src/ipc-client.ts          — IPC client (status/stop/quota/models/session_messages to agent process); AgentStatusResponse includes bootId field; PhysicalSessionSummary and SubagentInfo types; getAgentQuota(), getAgentModels(), and getAgentSessionMessages() functions
