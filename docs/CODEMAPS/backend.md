...
- `postToolUseFallback` method: fallback behavior when gateway is offline (RPC fails); gates on `copilotclaw_wait` to avoid injecting into subagent calls; performs pending message peek and injects notification into additionalContext
- `onPostToolUse` — registered via makeHookHandler; gateway-side logic gates on `copilotclaw_wait` (the sole parent-exclusive tool) to avoid injecting reminders/notifications into subagent tool calls (SDK hook system provides no mechanism to distinguish parent vs subagent calls); performs two checks:
