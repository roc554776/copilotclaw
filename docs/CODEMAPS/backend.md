...
- `onPostToolUse` — executes after each tool call; gates on `copilotclaw_receive_input` (the sole parent-exclusive tool) to avoid injecting reminders/notifications into subagent tool calls (SDK hook system provides no mechanism to distinguish parent vs subagent calls); performs two checks:
