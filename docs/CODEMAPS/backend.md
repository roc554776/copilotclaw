<!-- Generated: 2026-03-27 | Updated: 2026-03-28 | Files scanned: 43 | Version: 0.43.0 | Token estimate: ~4100 -->
...
```
Gateway vitest (245 tests)   — unit + E2E tests with mock agent (includes config, config-cli, config-migration, doctor, ipc-paths, setup, workspace, structured-logger, session-event-store, store tests)
Agent vitest (87 tests)      — unit tests with mock Copilot SDK session + IPC stream tests (includes structured-logger, token-resolver, ipc-stream tests)
