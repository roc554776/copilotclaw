<!-- Generated: 2026-03-27 | Updated: 2026-03-30 | Packages: 3 (cli, gateway, agent) | Version: 0.50.0 | Token estimate: ~2400 -->
...
- **Agent**: single process, executes physical sessions on behalf of gateway; communicates with gateway exclusively via IPC (stream for push-based messaging, short-lived connections for status/stop/quota/models); sendToGateway buffers messages to in-memory queue + JSONL file when stream disconnected (MAX_QUEUE_SIZE=10000), queue restored from disk on init via initSendQueue(dataDir), flushed on stream connect via flushSendQueue(); receives OTel config from gateway via IPC stream config push and initializes its own OTel setup independently; does not manage abstract sessions, channel bindings, backoff, or persistence (moved to gateway SessionOrchestrator in v0.49.0)
...
- Custom agent definitions (channel-operator, worker) and session timing config (staleTimeoutMs, maxSessionAgeMs, rapidFailureThresholdMs, backoffDurationMs, keepaliveTimeoutMs, reminderThresholdPercent, initialPrompt) defined in gateway's `agent-config.ts` as AgentPromptConfig and CustomAgentDef interfaces; resolveModel function for gateway-side model selection (v0.50.0); sent to agent via IPC config push
