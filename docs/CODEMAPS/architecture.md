<!-- Generated: 2026-03-27 | Updated: 2026-03-31 | Packages: 3 (cli, gateway, agent) | Version: 0.65.0 | Token estimate: ~2500 -->
...
- **SessionEventStore**: SQLite-based event storage (session-events.db in data dir, WAL mode); table: session_events (autoincrement id, sessionId, type, timestamp, data as JSON, parentId; indexed by sessionId, sessionId+timestamp, type); stores system prompt snapshots as JSON files in `{{stateDir}}/data/prompts/`; enforces configurable storage cap by row count (default 100k events) by deleting oldest rows every 500 inserts; getEventsPaginated(sessionId, limit, {before?, after?}) for cursor-based pagination by id; getEventCount(sessionId) returns total event count; getTokenUsage(from, to) aggregates assistant.usage events by model within time range (returns per-model totals of input/output tokens and multiplier); getTokenUsageTimeseries(from, to, points, movingAverageWindowSec) returns time-bucketed data with per-model usage, consumption index, and optional moving average
...
- **Token Usage Page** (`/token-usage`): recharts-based visualizations — token consumption index line chart with optional moving average overlay, stacked area chart for per-model token usage; period selector and moving average window controls; data from GET /api/token-usage/timeseries
...
- Agent version check: gateway enforces minimum agent version (MIN_AGENT_VERSION=0.65.0, exported from agent-manager.ts) at start; force-restart on mismatch with reconnectStream(); checkCompatibility()/getMinAgentVersion() expose compatibility status; CLI checkAgentCompatibility polls /api/status when waitForAgent=true (used after force-restart to wait for new agent bootId)
