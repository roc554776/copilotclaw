<!-- Generated: 2026-03-27 | Updated: 2026-04-16 | Packages: 3 (cli, gateway, agent) | Version: 0.79.0 | Token estimate: ~2600 -->
...
- **Binding Persistence (v0.49.0)**: SessionOrchestrator uses SQLite (session-orchestrator.db in data dir, WAL mode); abstract_sessions table (sessionId PK, channelId, status, startedAt, copilotSessionId, cumulativeInputTokens, cumulativeOutputTokens, physicalSessionHistory JSON); orchestrator_schema_version table tracks migration version; persistSession() upserts on every mutation; loadFromDb() on construction (restores physicalSession from history for idle sessions so they remain visible in UI); schema migration system (LATEST_SCHEMA_VERSION=3, SCHEMA_MIGRATIONS registry): initSchema creates base tables, runDataMigrations applies sequential migrations (v0→v1: suspended+channel+history → idle; v1→v2: remaining non-idle+channel → idle if history exists, → new if no history; v2→v3: fix idle+no-history → new); legacy one-time migration from agent-bindings.json (renames to .migrated after migration)
...
- Agent version check: gateway enforces minimum agent version (MIN_AGENT_VERSION=0.70.0, exported from agent-manager.ts) at start; force-restart on mismatch with reconnectStream(); checkCompatibility()/getMinAgentVersion() expose compatibility status; CLI checkAgentCompatibility polls /api/status when waitForAgent=true (used after force-restart to wait for new agent bootId)
...
- Test runners: vitest for unit + E2E (825 tests: 112 agent + 631 gateway vitest + 82 frontend), Playwright for browser E2E (26 tests); gateway vitest excludes test/browser/ directory
